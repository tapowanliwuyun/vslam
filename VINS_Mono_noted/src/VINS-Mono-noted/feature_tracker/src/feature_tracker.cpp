...
// 根据状态位，进行“瘦身”，双指针，理论最优解 0(n)
void F(vector<cv::Point2f> &v, vector<uchar> status)
{
    int j = 0;
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (cur_pts.size() > 0) // 上一帧有特征点，就可以进行光流追踪了
    {
        ...
        reduceVector(forw_pts, status); // 当前帧的特征点
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        TicToc t_t;
        int n_max_cnt = MAX_CNT - static_cast<int>(forw_pts.size());//  要提取的新特征点的数目 = 允许的最大特征点数目 - 当前存活下来的特征点数目
        if (n_max_cnt > 0)
        {
            if(mask.empty())//已经没有任何区域可以提取新的特征点
            ...
            /**
             * inputArray   image 原始图像，肯定在原始灰度图上提取
             * outputArray  corners 输出参数，提出的角点的坐标
             * int  maxCorners  最多提取多少个坐标
             * double qualityLevel  表示最小接受的质量，设置分数为1500分，如果该值为0.01，则小于15分的是不被接受的
             * double minDistance 每两个特征点之间的欧拉距离不能低于这个数
             * InputArray mask = noArray()  保证新提的特征点不会与所设置的保护区冲突
             * int blockSize = 3
