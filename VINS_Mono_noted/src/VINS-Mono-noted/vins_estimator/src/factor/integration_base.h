...
class IntegrationBase
{
  public:
    ...
    //jacobian 就是预积分量关于零偏的雅可比
    //covariance 协方差矩阵 在没有任何数据来临之前是0，置信度
    //sum_dt 
    //delta_p 位移的变化量
    //delta_q 伽马，用一个四元数来维护
    //delta_v 贝塔，速度的变化量
    {
    ...
    // 行参：IMU数据的时间戳、此时刻加速度的值、此时刻陀螺仪的值
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        //中值积分
        midPointIntegration(_dt, acc_0, gyr_0, _acc_1, _gyr_1, delta_p, delta_q, delta_v,//时间戳、k时刻加速度值、k时刻陀螺仪的值、k+1时刻加速度值、k+1时刻陀螺仪值、k时刻位移变量、k时刻旋转变量、k时刻速度变量
                            linearized_ba, linearized_bg,//两个零偏，零偏在两帧间保持不变
                            result_delta_p, result_delta_q, result_delta_v,//输出的参数：k+1时刻位移变量、k+1时刻旋转变量、k+1时刻速度变量
                            result_linearized_ba, result_linearized_bg, 1);//输出的两个零偏，没有改变，直接赋值输出
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        //对于预积分量，通过在预积分时求出的雅可比矩阵对其进行更新补偿，
